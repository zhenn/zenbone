            ...
            self.createProjectConfig(templateConfig, function (result) {
                fsextra.copy(path.resolve(currTemplate, 'template'), self.cwd, function (err) {
                    if (!err) {
                        ...
                        childProcess.execSync(`cd ${self.cwd} && npm install`);
                        // TODO 读取一下package中的postinit脚本
                        const postInit = (require(pkgConfig).scripts || {}).postinit;
                        if (postInit) {
                            const execPostInit = childProcess.execSync(`cd ${self.cwd} && ${postInit}`);
                            console.log(`postinit script execute success:\r\n${execPostInit}`);
                        }
