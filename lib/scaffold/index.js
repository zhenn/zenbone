module.exports = function () {
    ...
    return {
        cwd: process.cwd(),
        spinner: null,
        gitReposUrl: 'https://gitlab.pengpengla.com/template/<%=name%>.git',
        templateDir: templateDir,
        templateDirExist: fs.existsSync(templateDir),
        templateList: function () {
            const WordTable = require('word-table');
            return require('axios')
                .get('https://gitlab.pengpengla.com/api/v3/groups/133?private_token=Vc8tNijVAeKTAy66wueN')
                .then(function (resp) {
                ...
                .catch(function (e) {
                    if (e.response.data.message === '401 Unauthorized') {
                        ...
                        console.log('请访问：');
                        console.log('https://gitlab.pengpengla.com/profile/personal_access_tokens'.blue);
                    } else {
        ...
        createProject: function (templateName) {
            ...
            //  读取询问的配置文件
            //  兼容之前的老版本，所以这里需要做一些配置
            const templateConfigFile = path.resolve(currTemplate, 'config.json');
            const templateConfigJsFile = path.resolve(currTemplate, 'config.js');

            let templateConfig = {
                question: [],
                handler(fileAndConfig) {
                    // 这里面所有的都是同步操作，扔进去一个文件系统做处理
                }
            };

            // 优先读取config.js, config.json只会存储询问的问题
            if (fs.existsSync(templateConfigJsFile)) {
                templateConfig = require(templateConfigJsFile);
            } else if (fs.existsSync(templateConfigFile)) {
                templateConfig.question = require(templateConfigFile);
            }

            // 初始化默认值
            const {question = [], handler = e => e} = templateConfig;

            // 按照配置询问用户需要的字段
            self.createProjectConfig(question, function (result) {
                fsextra.copy(path.resolve(currTemplate, 'template'), self.cwd, function (err) {
                    if (!err) {
                        ...
                        // 在这里执行handler，对文件进行删除等操作者
                        handler({
                            underscore: _,
                            params: result,
                            directory: self.cwd,
                            files: fs.readdirSync(self.cwd)
                        });

                        console.log('log:'.green + (' 项目 "' + result.name + '" 创建成功。').gray);

                        console.log();
                        console.log('请执行下面代码安装依赖：');
                        console.log();
                        console.log('npm install -d'.yellow);
                        console.log();
                    }
                });
            });
