        ...
        main: function (opt) {
            ...
            function startBuild () {
                ...
                self.createAssets(opt, function (error) {
                    ...
                    console.log('\n你可能需要执行：');
                    if (env === 'product') {
                        console.log(`\n git add . && git commit -m "ci" && git push origin master && git tag publish/${pkgConfig.version} && git push origin publish/${pkgConfig.version}\n`.yellow);
                        console.log('执行完上面的命令，你可以使用下面的命令直接调用jenkins部署：');
                        console.log('\n zenbone deploy --product\n'.yellow);
                    } else if (opt.widget) {
                        console.log(`\n git add . && git commit -m "ci" && git push origin master\n`.yellow);
                    } else {
                        console.log(`\n git add . && git commit -m "ci" && git push origin daily/${pkgConfig.version}\n`.yellow);
                        console.log('执行完上面的命令，你可以使用下面的命令直接调用jenkins部署：');
                        console.log('\n zenbone deploy\n'.yellow);
                    }
                });
